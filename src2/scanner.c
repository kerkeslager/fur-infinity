...
Token Scanner_scan(Scanner* self) {
  for(;;) {
    switch(*(self->current)) {
      /* Completely ignore non-newline whitespace */
      case ' ':
      case '\r':
      case '\t':
        self->current++;
      ...
      /* Increment line on newline, but otherwise ignore */
      case '\n':
        self->line++;
        self->current++;
        break;
      ...
      case '(':
        {
          Token t;
          Token_init(&t, TOKEN_OPEN_PAREN, self->line, 1, self->current);
          self->current++;
          return t;
        }
      ...
      case ')':
        {
          Token t;
          Token_init(&t, TOKEN_CLOSE_PAREN, self->line, 1, self->current);
          self->current++;
          return t;
        }
